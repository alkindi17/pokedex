...
export default function RecentPokemons() {
  const [recentPokemons, setRecentPokemons] = useState<string[]>([]);

  useEffect(() => {
    setRecentPokemons(getRecentPokemons());
  }, []);

  if (!recentPokemons.length) return null;
    ...
    <div className="flex h-36 flex-col">
      ...
      <Suspense fallback={<LoadingDots />}>
        <ul className="flex h-28 items-center space-x-4 overflow-x-scroll px-4">
          {recentPokemons.map((pokemon: string) => (
